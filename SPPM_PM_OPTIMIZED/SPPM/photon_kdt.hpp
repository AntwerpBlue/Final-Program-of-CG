...
struct KDTNd
{
	...
	bool visited;
	int to_child; // -1 undef 0 lc 1 rc
	KDTNd() :ph(nullptr), axis(0), p(nullptr), lc(nullptr), rc(nullptr), visited(false), sibling(nullptr), to_child(-1) {};
};
...
public:
	...
	void non_recursive_kdt_search(pq_kdt& pq, int N, KDTNd* root, v3f& pos)
	{
		if (!root) return;
		std::stack<KDTNd*> traverse_stk;
		traverse_stk.push(root);

		while (!traverse_stk.empty()) 
		{
			auto nd = traverse_stk.top();
			if (!nd)
			{
				traverse_stk.pop();
				continue;
	...
	std::vector<Photon*> N_near(int N, v3f pos)
	{
		...
		non_recursive_kdt_search(pri_que, N, root, pos);
		int cnt = 0;
	...
	std::vector<Photon*> N_near_slow(int N, v3f pos)
	{
		pq_kdt pri_que([](const dist_kdtnd& a, const dist_kdtnd& b) {return a.first < b.first; });
